...
const orderedSports = (country, field, sortAscending) => {
  ...
  if(field)
  {
    if(sortAscending)
    {
      query = `SELECT sport, COUNT(*) AS count, (COUNT(sport) * 100 / (SELECT COUNT(*) FROM GoldMedal WHERE country = '${country}')) AS percent FROM GoldMedal WHERE country = '${country}' GROUP BY sport ORDER BY ASC;`
    }
    else
    {
      query = `SELECT sport, COUNT(*) AS count, (COUNT(sport) * 100 / (SELECT COUNT(*) FROM GoldMedal WHERE country = '${country}')) AS percent FROM GoldMedal WHERE country = '${country}' GROUP BY sport ORDER BY DESC;`
    }
  }
  return query;
};
